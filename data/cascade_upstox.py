...
class UpstoxPremiumSource:
    ...
    def _minute_series(
        ...
        if cached_rows is not None and not refresh:
            ...
            try:
                ...
                # caller can then report an invalid/expired token immediately
                # rather than spend hours producing an empty backtest.
    ...
    def release_memory(self) -> None:
        ...
        Long chronological replays visit many expiries. Keeping every raw
        one-minute contract dictionary alive expands an on-disk cache of
        hundreds of MB into several GB of Python objects. The selector calls
        this between expiries; a later lookup simply reopens the same cached
        JSON without another Upstox request.
        """
        self._series.clear()
