...
def cpr(df: pd.DataFrame, narrow_pct: float = 0.2,
        moderate_pct: float = 0.5, wide_pct: float = 0.5) -> pd.DataFrame:
    """Bug 2 fixed: handles both intraday and daily DataFrames."""
    intraday = _is_intraday(df)
    daily = df.resample("D").agg({"open":"first","high":"max","low":"min","close":"last"}).dropna() if intraday else df.copy()
    ...
    pivot_cols = ["pivot","bc","tc","cpr_width_pct","cpr_type",
...
def orb(df: pd.DataFrame, window_minutes: int = 15) -> pd.DataFrame:
    ...
    for date, group in result.groupby(result.index.date):
        ...
        if len(orb_candles) > 0:
            ...
            # Apply ORB levels to all candles of that day
            day_mask = result.index.date == date
            result.loc[day_mask, "ORB_High"] = orb_high
            result.loc[day_mask, "ORB_Low"]  = orb_low
