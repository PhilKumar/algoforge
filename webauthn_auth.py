"""
webauthn_auth.py — passkeys (Face ID / fingerprint) for PhilForge.

WHY THIS IS HAND-WRITTEN RATHER THAN A LIBRARY
The obvious choice is the `webauthn` package, but it requires cryptography>=49
and this app shares a virtualenv with CryptoForge, which is live trading real
money on a pinned cryptography. Forcing that upgrade to add a login convenience
is a bad trade. Everything needed here is a small, fully specified subset of
