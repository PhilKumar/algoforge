...
def _candle_entry_charts(engine: LadderCandleEntryPaper, batches: Mapping[str, list]) -> dict:
    ...
    `ladder_chart` already draws a TwoRedLadder: fills as
...
async def paper_campaign_chart(strategy: str, campaign_id: int, request: Request, timeframe: str = ""):
    ...
    the fib-boundary tab already uses. A campaign rebuilt from recorded prices kept no
...
async def candle_entry_paper_chart(request: Request, timeframe: str = ""):
    ...
    same `ladder_chart` the backtest draws with.
    """
    runtime = _candle_entry_engines.get(_request_user_id(request))
    if runtime is None:
...
def _gap_carry_trade_mode(value: str) -> str:
    ...
    if mode == "live" and not _OPTIONS_LIVE_EXECUTION_ENABLED:
        ...
        raise HTTPException(
            status_code=503,
            detail=(
                "Gap Carry live execution is built but disabled until its fills, partial fills and "
                "restart reconciliation are proven against Dhan. Use Paper or Backtest."
            ),
        )
    return mode
...
async def _start_gap_carry_campaign(user_id: int, payload, *, broker_client: DhanClient) -> _CascadeRuntime:
    ...
    # MARGIN, not INTRADAY: the whole rule is a contract held across ONE
...
def _nifty_lot_size_on(day: date) -> int:
    ...
    (the fib-boundary tab reads it) -- add new steps there, never here.
    """
