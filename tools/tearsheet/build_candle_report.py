...
<div class="note">
  ...
  <p>{
    t(
        "Every mother here is the high of the last 278 bars on the 5-minute chart &mdash; the rule picks it, not a trader &mdash; and campaigns are counted <strong>one at a time</strong>, exactly as the auto mother runs: a new box high that fires while a campaign is open is not taken; a campaign that ends without profit is followed at once by another on the same box high, watching from that moment; a profit waits for the next new high. A campaign that never saw its two reds never traded and is not in the book (counted above). What the tables measure is the <strong>rule</strong>, on the exact code the paper run executes, walked candle by candle in time order with real recorded premiums. Whether <em>your</em> campaigns earn is what the paper run is for.",
