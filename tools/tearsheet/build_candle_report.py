...
stepping down then a buy-stop on the first red's close, 5m -> 15m -> 1H with 1,
2, 3 lots, one NIFTY CE ATM-2 on the monthly, a quarter-way target that arms a
...
<section class="document-hero">
  <div class="hero-copy">
    ...
    <p class="lede">{
    t(
        "Phil's rule, measured as the page runs it. On the 5-minute chart the last 278 bars make a box; the <strong>mother</strong> is the bar that made the box high, and the ladder may buy only while price sits in the <strong>bottom quarter</strong> of that box. Two red closes stepping down put a buy-stop on the first red's close; the recovery is bought, 1 lot, and each buy moves the watch one chart up &mdash; 5m &rarr; 15m &rarr; 1H, 2 lots then 3 &mdash; after a new low. One NIFTY call, ATM&minus;2 on the monthly, for the whole ladder. The target is a quarter of the way from the average entry back to the mother's high; touching it arms a trail, and the basket is sold on a 5m close that gives back 30% of the gain. No stop loss, no time stop. Real recorded premiums, one campaign at a time, every box high from 1 Oct 2024 to 17 Aug 2026. <strong>Puts lost under the same rule and are not traded.</strong>",
        "Phil-இன் விதி, பக்கம் இயக்குவது போலவே அளக்கப்பட்டது. 5-நிமிட chart-இல் கடைசி 278 bars ஒரு box; box-இன் high-ஐ உருவாக்கிய bar-தான் <strong>mother</strong>; விலை அந்த box-இன் <strong>கீழ் கால் பகுதியில்</strong> இருக்கும்போது மட்டுமே ladder வாங்கும். இறங்கும் இரண்டு சிவப்பு close முதல் சிவப்பின் close-இல் buy-stop வைக்கும்; மீட்சி வாங்கப்படும், 1 lot, ஒவ்வொரு வாங்கலும் கண்காணிப்பை ஒரு chart மேலே நகர்த்தும் &mdash; 5m &rarr; 15m &rarr; 1H, 2 பின் 3 lots &mdash; புதிய low-க்குப் பின். முழு ladder-க்கும் ஒரே NIFTY கால், monthly-இல் ATM&minus;2. இலக்கு சராசரி நுழைவிலிருந்து mother-இன் high நோக்கி கால் பங்கு; அதைத் தொட்டதும் trail, லாபத்தில் 30% திரும்பக் கொடுக்கும் 5m close-இல் விற்பனை. Stop loss இல்லை, time stop இல்லை. உண்மையான premium-கள், ஒரு நேரத்தில் ஒரு campaign, 1 அக் 2024 &ndash; 17 ஆக 2026. <strong>புட் அதே விதியில் நஷ்டம், வர்த்தகம் இல்லை.</strong>",
    )
}</p>
    ...
    t(
        "1, 2, 3 lots up the ladder &middot; ATM&minus;2 monthly call",
        "Ladder-இல் 1, 2, 3 lots &middot; ATM&minus;2 monthly கால்",
    )
}</strong></div>
...
<div class="note">
  ...
  <p>{
    t(
        "Every mother here is a bar that made a new 278-bar high on the 5-minute chart &mdash; the rule picks it, not a trader &mdash; and campaigns are counted <strong>one at a time</strong>, exactly as the page runs: a new box high that fires while an earlier campaign is still open is not taken. A campaign that never saw its two reds never traded and is not in the book (counted above). What the tables measure is the <strong>rule</strong>, on the exact code the paper run executes, walked candle by candle in time order with real recorded premiums. Whether <em>your</em> campaigns earn is what the paper run is for.",
        "இங்குள்ள ஒவ்வொரு mother-உம் 5-நிமிட chart-இல் புதிய 278-bar high செய்த bar &mdash; விதி தேர்வு செய்கிறது, trader அல்ல &mdash; campaign-கள் <strong>ஒரு நேரத்தில் ஒன்று</strong> என கணக்கிடப்பட்டன, பக்கம் இயங்குவது போலவே: முந்தைய campaign திறந்திருக்கும்போது வரும் புதிய box high எடுக்கப்படாது. இரண்டு சிவப்பு வராத campaign வர்த்தகமே செய்யவில்லை, புத்தகத்தில் இல்லை (மேலே எண்ணப்பட்டது). அளக்கப்படுவது <strong>விதி</strong>, paper run இயக்கும் அதே code-இல், நேர வரிசையில், உண்மையான premium-களில். <em>உங்கள்</em> campaign-கள் சம்பாதிக்குமா என்பதுதான் paper run-இன் வேலை.",
    )
}</p>
  <p>{
    t(
        "Eleven independent checks were run before this was published: the backtest walk equals a tick-fed paper loop (2,173 checks); an audit written from the rule rather than the engine re-derives every mother, every fill, every premium and the money from the raw candle cache and the raw option archive (331 checks); a rerun is byte-identical; no campaign overlaps another; lot size follows the date; every expiry is 13&ndash;43 days out; no fill is priced outside its own bar; the muhurat session is excluded; every leg was repriced at the minute the index actually crossed its level and the trailing book came out <em>higher</em>, not lower; the same rule on puts, on 1m, with a bigger target, with a time stop or a stop loss was measured and lost. Zero failures.",
        "வெளியிடும் முன் பதினொரு சுயாதீன சரிபார்ப்புகள்: backtest நடை = tick-fed paper loop (2,173 சரிபார்ப்புகள்); engine-இல் அல்ல, விதியிலிருந்து எழுதப்பட்ட audit ஒவ்வொரு mother, fill, premium, பணத்தையும் மூல candle cache-இலும் மூல option archive-இலும் இருந்து மறு-வருவித்தது (331); மறு-இயக்கம் byte-அளவில் ஒன்றே; campaign-கள் மேற்பொருந்தவில்லை; lot தேதிப்படி; ஒவ்வொரு expiry-உம் 13&ndash;43 நாள்; எந்த fill-உம் அதன் bar-க்கு வெளியே விலையிடப்படவில்லை; muhurat அமர்வு நீக்கம்; ஒவ்வொரு leg-உம் index உண்மையில் கடந்த நிமிடத்தில் மறுவிலை, trailing புத்தகம் <em>உயர்ந்தது</em>; அதே விதி புட்-இல், 1m-இல், பெரிய இலக்குடன், time stop / stop loss-உடன் அளக்கப்பட்டு நஷ்டம். பூஜ்ஜியம் தோல்வி.",
    )
}</p>
</div>
...
<div class="note note-warn">
  ...
  <p>{
    t(
        f"The same rule in a mirror &mdash; the mother is the bar that makes the 278-bar LOW, two GREEN closes stepping up, a sell-stop on the first green's close, a put bought when price breaks back down, the box asking for the top quarter, ATM+2 &mdash; was replayed over the same 22 months: {PE['trades']} campaigns, {PE['wins']} won / {PE['losses']} lost, net {r(PE['net'])}, worst drawdown {r(PE['max_dd'])}. It wins more than half the time and still loses money; the losers are simply bigger, which is what a bought put does in a market that spent these two years grinding higher. Every put setting measured (the midpoint, 15m, 1H, fixed exit) lost as well. This sheet, and the page, is calls only.",
        f"அதே விதி கண்ணாடியில் &mdash; 278-bar LOW-ஐ உருவாக்கிய bar mother, மேலேறும் இரண்டு பச்சை close, முதல் பச்சையின் close-இல் sell-stop, விலை கீழே உடைந்தால் புட், box-இன் மேல் கால் பகுதி, ATM+2 &mdash; அதே 22 மாதங்களில்: {PE['trades']} campaign-கள், {PE['wins']} வெற்றி / {PE['losses']} நஷ்டம், நிகர {r(PE['net'])}, அதிகபட்ச இறக்கம் {r(PE['max_dd'])}. பாதிக்கும் மேல் வெற்றி, ஆனாலும் நஷ்டம் &mdash; நஷ்டங்கள் பெரியவை. அளந்த ஒவ்வொரு புட் அமைப்பும் நஷ்டம். இந்த அறிக்கையும் பக்கமும் கால் மட்டும்.",
    )
}</p>
</div>
...
<section>
  <div class="shead"><div><h2>{
    ...
    <p>{
    t(
        "Every buy is paid in full (no margin). The first rung is one lot of the ATM&minus;2 monthly call, the second two lots, the third three &mdash; so a fully built ladder holds six lots, and the deployed figures below sum what each campaign actually paid. Most campaigns stop at one or two rungs; the median is what a typical one needs, the maximum is what the deepest one needed.",
        "ஒவ்வொரு வாங்கலும் முழு premium (margin இல்லை). முதல் rung ATM&minus;2 monthly கால்-இன் ஒரு lot, இரண்டாவது இரண்டு, மூன்றாவது மூன்று &mdash; முழு ladder ஆறு lots. கீழே உள்ள எண்கள் ஒவ்வொரு campaign-உம் உண்மையில் செலுத்தியது. பெரும்பாலானவை ஒன்று அல்லது இரண்டு rung-இல் நிற்கும்.",
    )
}</p></div></div>
  <div class="tblwrap"><table>
